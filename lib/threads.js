...
export async function searchThreadsByProduct(product, manualQuery = "") {
  const token = await getThreadsToken();
  ...
  const queries = searchQueriesForProduct(product, manualQuery);
  const found = [];
  const seen = new Set();
...
function searchQueriesForProduct(product, manualQuery) {
  const keywords = splitList(product.keywords);
  const category = String(product.category || "").trim();
  const audience = String(product.audience || "").trim();
  const productWords = String(product.name || "").split(/\s+/).filter((word) => word.length > 3);
  const queries = [
    manualQuery,
    ...keywords,
    category,
    ...
    keywords[0] ? `rekomendasi ${keywords[0]}` : "",
    keywords[0] ? `butuh ${keywords[0]}` : "",
    audience ? `rekomendasi untuk ${audience}` : "",
    "rekomendasi barang",
    "lagi cari rekomendasi",
  ];
  return [...new Set(queries.map(normalizeSearchQuery).filter(Boolean))].slice(0, 12);
}
...
async function keywordSearch(query, accessToken) {
  const attempts = [
    ...
    () => {
      ...
      url.searchParams.set("fields", `keyword_search(q:"${query.replaceAll('"', '\\"')}"){id,text,username,permalink,timestamp}`);
      url.searchParams.set("access_token", accessToken);
      return url;
    },
    () => {
      const url = new URL("https://graph.threads.net/v1.0/me/keyword_search");
      url.searchParams.set("q", query);
      url.searchParams.set("fields", "id,text,username,permalink,timestamp");
      url.searchParams.set("access_token", accessToken);
      return url;
  ...
  for (const buildUrl of attempts) {
    try {
      const body = await fetch(buildUrl()).then(assertOk);
      if (body.keyword_search) return body.keyword_search;
      return body;
    } catch (error) {
      lastError = error;
    }
